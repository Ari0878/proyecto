...
<head>
    ...
    <title>Lista de Registros Sensor</title>
...
<body>
    ...
    <div class="container mt-5">
        <h2 class="mb-4">Lista de Registros de los Sensores</h2>

        <a href="{{ url('/alta-sensor') }}" class="btn btn-primary mb-4">
            <i class="bi bi-plus-circle"></i> Agregar Sensor
        </a>
        <a href="{{ route('sensorupload.excel.form') }}" class="btn btn-success mb-4">
            <i class="bi bi-upload"></i> Cargar Excel
        </a>
        <div class="d-flex justify-content-end mb-4">
            <a href="{{ route('pdf.sensor') }}" class="btn btn-primary">Generar PDF</a>
        </div>
                ...
                <tbody>
                    @foreach ($data as $sensor)
                        <tr>
                            ...
                            <td>{{ $sensor['altitud'] }}</td>
                            <td>{{ $sensor['velocidad'] }}</td>
                            <td>{{ $sensor['rumbo'] }}</td>
                            <td>{{ $sensor['vehiculo']['modelo'] }}</td>
                            <td>
                                <div class="btn-group" role="group">
                                    <a href="{{ url('/actualizar-sensor/' . $sensor['id']) }}" 
                                       class="btn btn-warning btn-sm" 
                                       title="Editar sensor">
                                        <i class="bi bi-pencil"></i> Editar
                                    </a>
                                    
                                    <a href="{{ url('/consultar2-sensor/' . $sensor['id']) }}" 
                                       class="btn btn-primary btn-sm"
                                       title="Ver detalles">
                                        <i class="bi bi-eye"></i> Detalle
                                    </a>
                                    
                                    <a href="{{ url('/borrar-sensor/' . $sensor['id']) }}" 
