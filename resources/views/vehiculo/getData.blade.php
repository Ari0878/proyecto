...
<body>
    <div class="container mt-5">
        <h2 class="mb-4">Lista de Registros de los vehiculos</h2>

        <!-- Botón para agregar un vehículo manualmente -->
        <a href="{{ url('/alta-vehi') }}" class="btn btn-primary mb-4">
            <i class="bi bi-plus-circle"></i> Agregar Vehiculo
        </a>

        <!-- Botón para cargar archivo Excel -->
        <a href="{{ route('upload.excel.form') }}" class="btn btn-success mb-4">
            <i class="bi bi-upload"></i> Cargar Excel
        </a>

        <!-- Botón para generar PDF -->
        <div class="d-flex justify-content-end mb-4">
            <a href="{{ route('pdf.vehiculo') }}" class="btn btn-primary">Generar PDF</a>
        </div>
            ...
            <table class="table table-bordered table-striped">
                <thead>
                    <tr>
                        ...
                        <th scope="col">Año</th>
                        <th scope="col">Año Alta</th>
                        <th scope="col">Sedema</th>
                ...
                <tbody>
                    @foreach ($data as $vehiculo)
                        <tr>
                            ...
                            <td>{{ $vehiculo['anio'] }}</td>
                            <td>{{ $vehiculo['anio_alta'] }}</td>
                            <td>{{ $vehiculo['sedema'] }}</td>
                            <td>{{ $vehiculo['tarjeta_circulacion'] }}</td>
                            <td>
                                <div class="btn-group" role="group" aria-label="Acciones de registro">
                                    <a href="{{ url('/actualizar-vehi/' . $vehiculo['id']) }}" 
                                       class="btn btn-warning btn-sm" 
                                       title="Editar vehiculo">
                                        <i class="bi bi-pencil"></i> Editar
                                    </a>
                                    <a href="{{ url('/consultar2-vehi/' . $vehiculo['id']) }}" 
                                       class="btn btn-primary btn-sm"
                                       title="Ver detalles">
                                        <i class="bi bi-eye"></i> Detalle
                                    </a>
                                    <a href="{{ url('/borrar-vehi/' . $vehiculo['id']) }}" 
